...
/**
 * @author SehamAlharbi The whole graph is represented by K keys - start -
 *         vertices and a list of their neighbours - Node objects
 * @param <K> vertices
...
public class Graph<K, E> {
	...
	/**
	 * overloaded recursive method that does the DFS algorithm implementation,
	...
	private boolean hasPathDFS(K source, K destination, HashSet<K> visited) {
		...
		for (Node<K, E> child : graph.get(source)) {
			if (hasPathDFS(child.endNode, destination, visited)) {
				return true;
			}
		}
